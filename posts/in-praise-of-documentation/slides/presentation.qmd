...
If you want to execute Python code cells in the document, Quarto uses `IPython` kernels. Set `QUARTO_PYTHON` to your project interpreter (for example the repo `.venv` after `uv sync`) so the `python3` kernel uses the same packages as your render.
:::
