...
This Quarto document was originally configured to render a PDF with Typst. To render the Typst version, on the command line, you type:

```
quarto render talk.qmd
```

For the website build, the bibliography is now handled by Pandoc using a BibTeX file (`refs.bib`) so that citations render correctly to HTML (and can also be used with standard PDF output), while the Typst/Hayagriva setup is reserved for a separate, standalone PDF render.
